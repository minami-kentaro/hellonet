...
static void hnet_host_bandwidth_throttle(HNetHost& host)
{
    ...
    bool needsAdjustment = host.bandwidthLimitedPeers > 0;

    if (elapsedTime < HNET_HOST_BANDWIDTH_THROTTLE_INTERVAL) {
        return;
    }
    ...
    while (peersRemaining > 0 && needsAdjustment) {
        ...
        throttle = (dataTotal <= bandwidth) ? HNET_PEER_PACKET_THROTTLE_SCALE : (bandwidth * HNET_PEER_PACKET_THROTTLE_SCALE) / dataTotal;
        ...
        for (size_t i = 0; i < host.peerCount; i++) {
            ...
            uint32_t peerBandwidth = peer.incomingBandwidth * elapsedTime / 1000;
            if (throttle * peer.outgoingDataTotal / HNET_PEER_PACKET_THROTTLE_SCALE <= peerBandwidth) {
                continue;
            }

            peer.packetThrottleLimit = peerBandwidth * HNET_PEER_PACKET_THROTTLE_SCALE / peer.outgoingDataTotal;
    ...
    peersRemaining = static_cast<uint32_t>(host.connectedPeers);
...
int32_t hnet_host_service(HNetHost& host, HNetEvent& event)
{
    ...
    if (HNET_TIME_DIFF(host.serviceTime, host.bandwidthThrottleEpoch) >= HNET_HOST_BANDWIDTH_THROTTLE_INTERVAL) {
        hnet_host_bandwidth_throttle(host);
    }
